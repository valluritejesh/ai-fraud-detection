...
class Claim(Base):
    ...
    # Original AI Scores (Immutable once generated)
    ai_risk_score = Column(Float, default=0.0)
    ai_risk_level = Column(String(20), default="UNASSESSED", index=True)

    # Human Investigator Override Scores (Stored separately)
    override_risk_score = Column(Float, nullable=True)
    override_risk_level = Column(String(20), nullable=True)

    # Final Effective Scores
    final_risk_score = Column(Float, default=0.0)
    final_risk_level = Column(String(20), default="UNASSESSED", index=True)
    ...
    top_signal = Column(String(100), nullable=True, default="PENDING_ANALYSIS")
    assigned_investigator = Column(String(100), nullable=True, default="Unassigned")
...
class Evidence(Base):
    ...
    provider_mode = Column(String(100), default="LOCAL DEMO / MOCK")
...
class InvestigationCase(Base):
    ...
    assigned_to = Column(String(100), nullable=True, default="SIU Lead Investigator")
    priority = Column(String(20), default="HIGH")
    
    investigator_notes = Column(JSON, default=list)
    
    # Explicit separation of AI score vs override
    original_ai_score = Column(Float, default=0.0)
    original_ai_level = Column(String(20), default="UNASSESSED")
    ai_risk_overridden = Column(Boolean, default=False)
    override_score = Column(Float, nullable=True)
    override_reason = Column(Text, nullable=True)
    final_effective_score = Column(Float, default=0.0)
