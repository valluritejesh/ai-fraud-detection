...
export const ClaimDetailView: React.FC<ClaimDetailViewProps> = ({
  ...
  const getScoreTheme = (level: string) => {
    switch (level) {
      ...
      case "MEDIUM":
        return {
          badge: "bg-gold-50 text-gold-900 border-gold-300",
          bar: "bg-gold-500",
          glow: "border-gold-500/30",
        };
      default:
        return {
  ...
  const theme = getScoreTheme(effectiveLevel);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <button
          onClick={onBack}
          className="flex items-center space-x-2 text-forest-800 hover:text-emerald-950 px-3 py-1.5 rounded-xl hover:bg-cream-400 transition text-xs font-bold w-fit border border-transparent hover:border-cream-600"
        >
        ...
        <div className="flex items-center space-x-3">
          <span className="text-xs text-forest-700 font-medium">Status:</span>
          <span className="px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider bg-cream-400 text-emerald-950 border border-cream-700">
            {claim.status}
          </span>
          <button
            onClick={handleExternalSync}
            disabled={syncLoading}
            className="flex items-center space-x-1.5 px-3 py-1 bg-white hover:bg-cream-300 text-forest-900 border border-cream-700 rounded-lg text-xs font-bold transition shadow-sm"
      ...
      {/* Security Alert Banner (For Scenario F Prompt Injection) */}
      {hasPromptInjection && (
        <div className="p-4 rounded-2xl bg-rose-50 border-2 border-rose-400 text-rose-950 shadow-md flex items-start space-x-3">
          <AlertOctagon className="w-5 h-5 text-risk-critical shrink-0 mt-0.5" />
          <div className="text-xs space-y-1">
            <div className="flex items-center space-x-2">
              <span className="font-extrabold uppercase tracking-wide text-risk-critical">
                SECURITY THREAT DETECTED:
              </span>
              <span className="bg-rose-200 text-rose-900 font-bold px-1.5 py-0.2 rounded text-[10px]">
                INDIRECT PROMPT INJECTION SANITIZED
              </span>
            </div>
            <p className="font-medium text-rose-900">
              The Document Agent neutralized embedded instructions: <code className="bg-rose-100 px-1 py-0.5 rounded font-mono">[REDACTED_SECURITY_THREAT]</code>. Threat was quarantined, and risk was escalated to SIU.
            </p>
          </div>
        </div>
      )}

      {/* Top Red Flag Banner (If Present) */}
      {claim.top_signal && !hasPromptInjection && (
        <div className="bg-amber-50 border border-amber-300 rounded-2xl p-3.5 flex items-start space-x-3 shadow-sm">
          <AlertTriangle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
          <div className="text-xs">
            <span className="font-bold text-amber-900 uppercase tracking-wide mr-2">Top Red Flag Signal:</span>
            <span className="text-emerald-950 font-semibold">{claim.top_signal}</span>
          </div>
        </div>
      )}

      {/* Hero Claim Dossier Card */}
      <div className="bg-white/95 backdrop-blur-md border border-cream-700/80 rounded-2xl p-6 shadow-card-soft relative overflow-hidden">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          {/* Left Metadata */}
          <div className="space-y-2">
            <div className="flex items-center space-x-3">
              <h1 className="text-2xl font-extrabold text-emerald-950 tracking-tight font-sans">
                {claim.id}
              </h1>
              <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase border ${theme.badge}`}>
                {effectiveLevel} RISK
              </span>
              {overrideScoreVal !== null && overrideScoreVal !== undefined && (
                <span className="bg-purple-100 text-purple-900 border border-purple-300 px-2.5 py-0.5 rounded-full text-[10px] font-bold flex items-center space-x-1">
            ...
            <p className="text-xs text-forest-800 flex flex-wrap items-center gap-2">
              <span className="font-bold text-emerald-950 text-sm">{claim.claimant_name}</span>
              <span className="text-cream-700">•</span>
              <span>Policy: <strong className="font-mono text-emerald-950">{claim.policy_id}</strong></span>
              <span className="text-cream-700">•</span>
              <span>Claimant ID: <strong className="font-mono text-emerald-950">{claim.claimant_id}</strong></span>
              {claim.assigned_investigator && (
                <>
                  <span className="text-cream-700">•</span>
                  <span className="text-emerald-800 font-semibold">Assigned: {claim.assigned_investigator}</span>
                </>
              )}
            </p>

            <div className="flex flex-wrap gap-4 text-xs text-forest-700 pt-1">
          ...
          {/* Explicit 3-Tier Separated Score Hierarchy */}
          <div className="flex flex-wrap sm:flex-nowrap gap-3 items-stretch">
            {/* 1. Original AI Score */}
            <div className="bg-cream-200/80 border border-cream-700/80 rounded-xl p-3.5 min-w-[150px] flex flex-col justify-between shadow-inner">
              <div className="flex items-center justify-between text-forest-800 text-[10px] uppercase font-extrabold tracking-wider mb-1">
                <span className="flex items-center space-x-1">
                  <Cpu className="w-3 h-3 text-emerald-700" />
                  <span>Original AI</span>
                </span>
                <span className="px-1.5 py-0.2 rounded text-[9px] font-bold bg-white border border-cream-700">
                  {aiLevel}
                </span>
              </div>
              <div className="text-2xl font-black text-emerald-950 my-1 font-sans">
                {aiScore.toFixed(0)}<span className="text-xs text-forest-700 font-normal"> / 100</span>
              </div>
              <div className="text-[10px] text-forest-700 font-mono">
                Immutable Model
              </div>
            </div>

            {/* 2. Human Override */}
            <div className="bg-cream-200/80 border border-purple-200 rounded-xl p-3.5 min-w-[150px] flex flex-col justify-between shadow-inner">
              <div className="flex items-center justify-between text-purple-900 text-[10px] uppercase font-extrabold tracking-wider mb-1">
                <span className="flex items-center space-x-1">
              ...
              <div className="text-2xl font-black text-purple-950 my-1 font-sans">
                {overrideScoreVal !== null && overrideScoreVal !== undefined ? (
                  <>{overrideScoreVal.toFixed(0)}<span className="text-xs text-purple-700 font-normal"> / 100</span></>
                ) : (
                  <span className="text-xs text-forest-700 font-medium">None (Using AI)</span>
                )}
              </div>
              <div className="text-[10px] text-forest-700 truncate max-w-[130px]" title={investigationCase?.override_reason || "No manual override"}>
                {investigationCase?.override_reason || "Automated Risk Active"}
              </div>
            </div>

            {/* 3. Effective Final Risk Gauge */}
            <div className="bg-forest-900 border border-emerald-700/60 text-white rounded-xl p-3.5 min-w-[190px] flex flex-col justify-between shadow-md">
              <div className="flex items-center justify-between text-emerald-200 text-[10px] uppercase font-extrabold tracking-wider mb-1">
                <span className="flex items-center space-x-1">
              ...
              <div className="flex items-baseline justify-between my-1">
                <span className="text-3xl font-black text-white">{effectiveScore.toFixed(0)}<span className="text-xs text-emerald-300/60 font-normal"> / 100</span></span>
                <span className="px-2 py-0.5 rounded text-[10px] font-black border border-white/20 bg-white/10 uppercase">
                  {effectiveLevel}
                </span>
              </div>
              <div className="w-full bg-emerald-950 h-1.5 rounded-full overflow-hidden mt-1">
                <div
        ...
        {[
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2 py-3 px-3 border-b-2 font-bold text-xs transition ${
                isActive
            ...
            >
              ...
              {tab.badge !== undefined && (
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
                  isActive ? "bg-forest-800 text-gold-200" : "bg-cream-400 text-forest-800"
                }`}>
                  {tab.badge}
                </span>
              )}
      ...
      {/* Tab 1: AI Findings & Structured Signals */}
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            {/* AI Explanation Callout */}
            {riskAssessment && (
              <div className="bg-white/95 backdrop-blur-md border border-cream-700/80 rounded-2xl p-5 space-y-3 shadow-card-soft">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-emerald-950 flex items-center space-x-2">
                    <SlidersHorizontal className="w-4 h-4 text-emerald-700" />
                    <span>Explainable Risk Assessment</span>
                  </h3>
                  <span className="text-xs font-semibold px-2.5 py-0.5 rounded bg-forest-800/10 text-forest-800 border border-forest-800/20">
                    Action: {riskAssessment.recommended_action}
                  </span>
                </div>
                <div className="p-4 rounded-xl bg-cream-300/40 border border-cream-600/70 font-mono text-xs text-emerald-950 whitespace-pre-wrap leading-relaxed">
              ...
              {(!claim.fraud_signals || claim.fraud_signals.length === 0) ? (
                ...
                claim.fraud_signals.map((sig) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${getScoreTheme(sig.severity).badge}`}>
                          {sig.severity}
                        </span>
                        <span className="text-xs font-semibold text-forest-800 font-mono">[{sig.category}]</span>
                        <span className="text-xs font-bold text-emerald-950">{sig.signal_type}</span>
                      </div>
                      <span className="text-xs font-extrabold text-rose-700">+{sig.score_impact} pts</span>
                    </div>
                    <p className="text-xs text-forest-900 leading-relaxed">{sig.description}</p>

                    {/* Structured Rule Metadata Details */}
                    {sig.metadata_json && sig.metadata_json.rule_id && (
                      <div className="p-3 rounded-xl bg-cream-300/50 border border-cream-600 text-xs space-y-1">
                        ...
                        {sig.metadata_json.explanation && (
                          <div className="text-[11px] text-forest-800 italic">
                            {sig.metadata_json.explanation}
                          </div>
                        )}
                      </div>
          ...
          <div className="space-y-6">
            <div className="bg-white/95 backdrop-blur-md border border-cream-700/80 rounded-2xl p-5 space-y-4 shadow-card-soft">
                  ...
                  {Object.entries(riskAssessment.score_breakdown).map(([k, v]) => (
                    <div key={k} className="space-y-1">
                      <div className="flex justify-between text-forest-900">
                        ...
                        <span className="font-bold text-emerald-950">{v} pts</span>
                      </div>
            ...
            <div className="bg-white/95 backdrop-blur-md border border-cream-700/80 rounded-2xl p-5 space-y-3 shadow-card-soft">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between py-1 border-b border-cream-600/50">
                  ...
                  <span className="text-emerald-950 font-bold">${claim.estimated_vehicle_value?.toLocaleString()}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-cream-600/50">
                  <span className="text-forest-700">Claim / Value Ratio:</span>
                  <span className={`font-bold ${((claim.claimed_amount / (claim.estimated_vehicle_value || 1)) > 0.8) ? "text-rose-700" : "text-emerald-700"}`}>
                    {((claim.claimed_amount / (claim.estimated_vehicle_value || 1)) * 100).toFixed(1)}%
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b border-cream-600/50">
                  <span className="text-forest-700">Incident Description:</span>
                  <span className="text-emerald-950 max-w-[180px] truncate text-right font-medium" title={claim.incident_description || "N/A"}>
      ...
      {activeTab === "evidence" && (
        <div className="space-y-6">
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs font-bold text-emerald-950 truncate max-w-[140px]">{ev.filename}</span>
                  </div>
                  <span className="text-[10px] bg-cream-400 text-forest-900 px-2 py-0.5 rounded font-mono font-semibold">
                    {ev.document_type}
                  </span>
                </div>
                <div className="text-[11px] text-forest-800 space-y-1">
                  <p>SHA-256: <span className="font-mono text-[9px] text-forest-700">{ev.sha256_hash.slice(0, 16)}...</span></p>
                  <p>Confidence: <span className="font-bold text-emerald-800">{(ev.confidence * 100).toFixed(0)}%</span></p>
                  <div className="flex items-center justify-between pt-1 border-t border-cream-600/50">
                    <span className="text-emerald-950 font-bold">{ev.extraction_status}</span>
                    <span className="text-[9px] bg-cream-400 text-forest-900 px-1.5 py-0.5 rounded border border-cream-600 font-semibold">
          ...
          {selectedEvidence && (
            <div className="bg-white/95 border border-cream-700/80 rounded-2xl p-5 space-y-4 shadow-card-soft">
              <div className="flex items-center justify-between border-b border-cream-600/60 pb-3">
                <div>
                  <h4 className="text-sm font-bold text-emerald-950 flex items-center space-x-2">
                    ...
                    <span className="text-xs font-mono bg-forest-800 text-gold-300 px-2 py-0.5 rounded font-bold">
                      {selectedEvidence.id}
                    </span>
                  </h4>
                ...
                <button
                  ...
                  className="text-xs text-forest-700 hover:text-emerald-950 font-bold"
                >
                  Close
                </button>
              </div>
      ...
      {activeTab === "investigation" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white/95 border border-cream-700/80 rounded-2xl p-5 space-y-4 shadow-card-soft">
            ...
            <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
              {(!investigationCase?.investigator_notes || investigationCase.investigator_notes.length === 0) ? (
                ...
                investigationCase.investigator_notes.map((n, i) => (
                  <div key={i} className="p-3 bg-cream-300/40 rounded-xl border border-cream-600 text-xs space-y-1">
                    <div className="flex justify-between text-forest-800 font-medium">
                      <span className="text-forest-900 font-bold">{n.author}</span>
                      <span className="text-[10px] text-forest-700 font-mono">{n.timestamp ? new Date(n.timestamp).toLocaleString() : ""}</span>
                    </div>
                    <p className="text-emerald-950">{n.text}</p>
                  </div>
                ))
              )}
            </div>
            ...
            <form onSubmit={handleAddNote} className="pt-2 border-t border-cream-600 space-y-2">
              <textarea
                ...
                rows={2}
                className="w-full bg-cream-300/40 border border-cream-600 rounded-xl px-3 py-2 text-xs text-emerald-950 placeholder-forest-800/50 focus:outline-none focus:ring-2 focus:ring-emerald-700/30 focus:border-emerald-700 focus:bg-white"
              />
              <button
                type="submit"
                disabled={actionLoading}
                className="flex items-center space-x-1.5 px-3 py-1.5 bg-forest-800 hover:bg-emerald-900 text-gold-200 hover:text-white rounded-xl text-xs font-bold transition shadow-sm"
              >
          ...
          <div className="space-y-6">
            {/* AI Risk Override */}
            <div className="bg-white/95 border border-cream-700/80 rounded-2xl p-5 space-y-3 shadow-card-soft">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-bold uppercase tracking-wider text-emerald-950">
                  Override AI Risk Assessment
                </h3>
                <span className="text-[10px] bg-purple-100 text-purple-900 px-2 py-0.5 rounded-full border border-purple-200 font-bold">
              ...
              <form onSubmit={handleOverrideScore} className="space-y-3 pt-1">
                <div>
                  <label className="text-xs font-bold text-forest-900 block mb-1">
                    New Effective Score (0 - 100): <span className="text-purple-800">{overrideScore}</span>
                  </label>
                  <input
                    type="range"
                    min="0"
                    ...
                    className="w-full h-2 bg-cream-400 rounded-lg cursor-pointer accent-forest-800"
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-forest-900 block mb-1">
                ...
                <button
                  type="submit"
                  disabled={actionLoading}
                  className="px-3.5 py-1.5 bg-purple-800 hover:bg-purple-900 text-white rounded-xl text-xs font-bold transition shadow-sm"
                >
                  Save Override
                </button>
              </form>
            ...
            <div className="bg-white/95 border border-cream-700/80 rounded-2xl p-5 space-y-3 shadow-card-soft">
              ...
              {investigationCase?.final_decision ? (
                <div className="p-3 bg-cream-300/50 rounded-xl border border-cream-600 text-xs space-y-1">
                  <span className="font-extrabold text-emerald-900">Decision: {investigationCase.final_decision}</span>
                  <p className="text-forest-900">Reason: {investigationCase.decision_reason}</p>
                  <p className="text-forest-700 text-[10px] font-mono">Decided by: {investigationCase.decided_by}</p>
                </div>
              ) : (
                <div className="flex flex-wrap gap-2 pt-2">
                  <button
                    ...
                    disabled={actionLoading}
                    className="flex items-center space-x-1 px-3 py-2 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl text-xs font-bold transition shadow-sm"
                  >
                  ...
                  <button
                    ...
                    disabled={actionLoading}
                    className="flex items-center space-x-1 px-3 py-2 bg-rose-700 hover:bg-rose-800 text-white rounded-xl text-xs font-bold transition shadow-sm"
                  >
                  ...
                  <button
                    ...
                    disabled={actionLoading}
                    className="flex items-center space-x-1 px-3 py-2 bg-amber-700 hover:bg-amber-800 text-white rounded-xl text-xs font-bold transition shadow-sm"
                  >
          ...
          {loadingAudit ? (
            ...
            <div className="space-y-3">
              {auditLogs.map((log) => (
                <div
                  key={log.id}
                  className="p-3 bg-cream-300/40 rounded-xl border border-cream-600 flex items-start justify-between text-xs"
                >
                  <div className="space-y-1">
                    <div className="flex items-center space-x-2">
                      <span className="font-mono text-emerald-900 font-bold">{log.action}</span>
                      <span className="text-cream-700">•</span>
